...
sub stream_start
{
  ...
  while($try <= 3) {
    ...
    my $cmd = "/usr/bin/raspivid -o - -t 0 -vf -hf -b 3000000 -fps 30 -rot $rot -w 1280 -h 720 -a 1036 | ffmpeg -loglevel panic -re -ar 44100 -ac 2 -acodec pcm_s16le -f s16le -ac 2 -i /dev/zero -f h264 -i - -vcodec copy -acodec aac -ab 128k -g 50 -strict experimental -f flv rtmp://a.rtmp.youtube.com/live2/$key";
