...
sub shoot
{
  ...
  my $ss;
  if($dt->epoch < $sun->sunrise_datetime($dt)->epoch) {
    if($dt->epoch > ($sun->sunrise_datetime($dt)->epoch - 7200)) {
      print "\nsunrising...\n";
      open(SS, "/home/pi/vals/ss");
      $sec = <SS>;
      close(SS);
      chomp $sec;
      $ss = ($sec * 1000000) - 100000;
      ## this is the floor
      undef $ss if($ss < 100000);
    }
    else {
      print "\nhard night...\n";
      $ss = 5000000;
  ...
  elsif($dt->epoch > $sun->sunset_datetime($dt)->epoch) {
    if($dt->epoch < ($sun->sunset_datetime($dt)->epoch + 3600)) {
      print "\nsunsetting...\n";
      open(SS, "/home/pi/vals/ss");
      $sec = <SS>;
      close(SS);
      chomp $sec;
      if($dt->epoch < ($sun->sunset_datetime($dt)->epoch + 900)) {
        $ss = ($sec * 1000000) + 5000;
      }
      elsif($dt->epoch < ($sun->sunset_datetime($dt)->epoch + 1800)) {
        $ss = ($sec * 1000000) + 15000;
      }
      elsif($dt->epoch < ($sun->sunset_datetime($dt)->epoch + 2700)) {
        $ss = ($sec * 1000000) + 50000;
      }
      else {
        $ss = ($sec * 1000000) + 200000;
      }
      ## here is the roof
      $ss = 5000000 if($ss > 5000000);
    }
    else {
      print "\nhard night...\n";
      $ss = 5000000;
