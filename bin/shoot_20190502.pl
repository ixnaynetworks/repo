...
sub stream_start
{
  ...
  while($try <= 3) {
    ...
    ## add logo for newest pi model
    if($conf->pi_model() eq "a020d3: 3 Model B+ // 1400 MHz // 1gb")
    {
...
## write value after everything is done?
  ...
  ## stamp
  #my $cmd = "/home/pi/bin/format_20190228.pl /home/pi/raw/$file $w $h $logo";
  #print "cmd=$cmd\n";
  #my $out = `$cmd`;
  #print "out=$out\n";
  #print "\n";
  $conf->format("/home/pi/raw/$file", $w, $h, $logo);
...
#  print "\n";
  ...
  my $cmd = "/bin/cp /home/pi/arc/$file /home/pi/upload/$cam\_$file ; /usr/bin/rsync --timeout=10 -avz --remove-source-files /home/pi/upload/$cam\_$file uaws:www/vhosts/ixnay/htdocs/cams/upload";
  print "cmd=$cmd\n";
  my $out = `$cmd`;
  print "out=$out\n";
  print "\n";
  ...
  #
  # failed upload? i know it failed because it is still sitting there.  move the file to the rsync dir
  # 

  if(-e "/home/pi/upload/$cam\_$file")
  {
    mkdir("/home/pi/rsync") unless(-e "/home/pi/rsync");

    my $cmd = "/bin/mv /home/pi/upload/$cam\_$file /home/pi/rsync/$file";
    print "cmd=$cmd\n";
    my $out = `$cmd`;
    print "out=$out\n";
    print "\n";
